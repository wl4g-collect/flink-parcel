...
base_dir="$(cd `dirname $0`/;pwd)/build"; mkdir -p ${base_dir}
flink_service_name="flink"
flink_service_name_lower="$(echo $flink_service_name | tr '[:upper:]' '[:lower:]')"
flink_archive="${base_dir}/$(basename $FLINK_URL)"
flink_unzip_folder="${base_dir}/${flink_service_name_lower}-${FLINK_VERSION}"
flink_folder_lower="$(basename $flink_archive .tgz)"
flink_parcel_folder="${base_dir}/$(echo $flink_folder_lower | tr '[:upper:]' '[:lower:]')"
flink_parcel_name="$flink_parcel_folder-el${OS_VERSION}.parcel"
flink_built_folder="${base_dir}/${flink_parcel_folder}"
flink_csd_build_folder="${base_dir}/flink_csd"
...
function get_flink {
  ...
  if [ ! -d "$flink_unzip_foleder" ]; then
    mkdir -p ${flink_unzip_folder}
    tar -xvf $flink_archive --strip-components=1 -C ${flink_unzip_folder}
  fi
}
...
function build_flink_parcel {
  ...
  local build_flink_parcel_folder="${flink_parcel_folder}/lib/${flink_service_name_lower}"
  mkdir -p $build_flink_parcel_folder/bin
  cp -r flink-parcel-src/flink-master.sh ${build_flink_parcel_folder}/bin
  cp -r flink-parcel-src/flink-worker.sh ${build_flink_parcel_folder}/bin
  cp -r flink-parcel-src/flink-yarn.sh ${build_flink_parcel_folder}/bin
  sed -i -e "s#%flink_version%#$flink_parcel_folder#g" ${flink_parcel_folder}/meta/flink_env.sh
  sed -i -e "s#%VERSION%#$FLINK_VERSION#g" ${flink_parcel_folder}/meta/parcel.json
  sed -i -e "s#%EXTENS_VERSION%#$EXTENS_VERSION#g" ${flink_parcel_folder}/meta/parcel.json
  sed -i -e "s#%CDH_MAX_FULL%#$CDH_MAX_FULL#g" ${flink_parcel_folder}/meta/parcel.json
  sed -i -e "s#%CDH_MIN_FULL%#$CDH_MIN_FULL#g" ${flink_parcel_folder}/meta/parcel.json
  sed -i -e "s#%SERVICENAME%#$flink_service_name#g" ${flink_parcel_folder}/meta/parcel.json
  sed -i -e "s#%SERVICENAMELOWER%#$flink_service_name_lower#g" ${flink_parcel_folder}/meta/parcel.json
